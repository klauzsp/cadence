...
import {RebalanceVault} from "./RebalanceVault.sol";

/// @notice Strategy-specific deployer registered with the primary VaultFactory.
contract RebalanceStrategyFactory is IStrategyFactory {
    error OnlyVaultFactory();
    error InvalidConfiguration();
    error UnsupportedToken(address token);

    bytes32 public constant override strategyId = keccak256("REBALANCE_V1");
    ...
    /// @dev initData = abi.encode(asset, targetToken, targetAllocationBps, thresholdBps, interval, maxSlippageBps, name, symbol).
    function createStrategy(address, bytes calldata initData) external returns (address vault) {
        ...
        ) = abi.decode(initData, (IERC20, IERC20, uint16, uint16, uint256, uint16, string, string));
